...
require_relative '../../../app/api_models/inventory/reserve.rb'
...
RSpec.describe Reserve, type: :model do
    describe 'validation' do
        before(:each) do
            ...
            amount         = 5
            @it            = Reserve.new(inventory_item, amount)
        end
    ...
    describe 'when running update_db() to update the database' do    
        context 'when everything is good' do
            before(:each) do
                ...
                @amount_to_reserve         = 5
                @it                        = Reserve.new(@inventory_item, @amount_to_reserve)
            ...
            it 'should add to the reserved amount by the specified amount' do
                expect(@inventory_item.reserved_amount).to eq(@original_reserved_amount + @amount_to_reserve)
            end
            it 'should remove the specified amount from the available amount' do
                expect(@inventory_item.available_amount).to eq(@original_available_amount - @amount_to_reserve)
            end
        ...
        context 'when not valid' do
            before(:each) do
                ...
                @original_reserved_amount  = @inventory_item.reserved_amount
                @original_available_amount = @inventory_item.available_amount
                @it                        = Reserve.new(@inventory_item, -1)
                @operation_result          = @it.update_db()
                @inventory_item.reload()
            end
        ...
        context 'when trying to reserve more than is available' do
            before(:each) do
                @inventory_item            = FactoryBot.create(:inventory, :available_amount => 5)
                @original_reserved_amount  = @inventory_item.reserved_amount
                @original_available_amount = @inventory_item.available_amount
