...
require_relative '../../../app/api_models/inventory/move_reserved_back_to_available.rb'
require_relative './shared_examples.rb'

RSpec.describe MoveReservedBackToAvailable, type: :model do
    describe 'validation' do
        before(:each) do
            inventory_item = FactoryBot.create(:inventory)
            amount         = 5
            @it            = MoveReservedBackToAvailable.new(inventory_item, amount)
        end
    ...
    describe 'when running update_db() to update the database' do    
        context 'when everything is good' do
            before(:each) do
                ...
                @amount_to_move            = 5
                @it                        = MoveReservedBackToAvailable.new(@inventory_item, @amount_to_move)
            ...
            it 'should add to available amount by the specified amount' do
                expect(@inventory_item.available_amount).to eq(@original_available_amount + @amount_to_move)
            end
            it 'should remove from the reserved amount by the specified amount' do
                expect(@inventory_item.reserved_amount).to eq(@original_reserved_amount - @amount_to_move)
            end
        ...
        context 'when not valid' do
            before(:each) do
                ...
                @original_reserved_amount  = @inventory_item.reserved_amount
                @original_available_amount = @inventory_item.available_amount
                @it                        = MoveReservedBackToAvailable.new(@inventory_item, -1)
                @operation_result          = @it.update_db()
                @inventory_item.reload()
            end
        ...
        context 'when trying to move more than is available' do
            before(:each) do
                @inventory_item            = FactoryBot.create(:inventory, :reserved_amount => 5)
                @original_reserved_amount  = @inventory_item.reserved_amount
                @original_available_amount = @inventory_item.available_amount
